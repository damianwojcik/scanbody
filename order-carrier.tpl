	...
	{else}
		...
		{else}
			...
			{if $conditions AND $cms_id}
				<p class="carrier_title carrier_title--large">{l s='Terms of service'}</p>
				<p class="checkbox checkbox--large">
					<input type="checkbox" name="cgv" id="cgv" value="1" {if $checkedTOS}checked="checked"{/if} />
					<label for="cgv">{l s='I agree to the terms of service and privacy policy and will adhere to them unconditionally.'}</label>
					<br>
					<a href="{$link_conditions|escape:'html':'UTF-8'}" class="iframe" rel="nofollow">{l s='Read the Terms of Service'}</a>
					<br>
					<a href="{$link->getCMSLink(9)|escape:'html':'UTF-8'}" class="iframe" rel="nofollow">{l s='Read the privacy policy.'}</a>
				</p>
			{/if}
		</div> <!-- end delivery_options_address -->
		{if !$opc}
